 ...
 * @param {Vec2} v
 * @return {Vec2} //changed this so that we weren't losing information for the line-distance function
 */
Vec2.prototype.sub = function (v)
{
    //todo needs to be implemented
	var v = new Vec2([this.array[0]-v.array[0],this.array[1]-v.array[1]]);
    return v;	 
};
...
{	//right now it only computes alpha,beta,gamma and stores them in a 3 element array
	var alpha,beta,gamma=0;
	//compute the a,b,g values
	alpha = pointLineDist(p2,p1,p)/pointLineDist(p2,p1,p0);
	beta = pointLineDist(p0,p2,p)/pointLineDist(p0,p2,p1);
	gamma = pointLineDist(p0,p1,p)/pointLineDist(p0,p1,p2);
...
function pointLineDist(p0, p1, p)
{
     ...
     //todo needs to be implemented
	 //need to test, this appears to be whats going on with the algorithm from the slides
	 var pt0 = new Vec2([p0[0],p0[1]]);
	 var pt1 = new Vec2([p1[0],p1[1]]);
	 var pt = new Vec2([p[0],p[1]]);
	 //substraction variables for readability
	 var v1 = pt1.sub(pt0);
	 var v2 =  pt0.sub(pt);
	 var mt = new Mat2();
		mt.set(0,0,v1.x); mt.set(1,0,v1.y);
		mt.set(0,1,v2.x); mt.set(1,1,v2.y);
	 var denominator =(pt1.sub(pt0)).mag();
	 d = (Math.abs(mt.det())/Math.abs(denominator));
	 
	 //	 ((mt.det)/(Math.abs(denominator.mag())));
